package com.entity.view;

import org.apache.tools.ant.util.DateUtils;
import com.annotation.ColumnInfo;
import com.entity.KuandaiEntity;
import com.baomidou.mybatisplus.annotations.TableName;
import org.apache.commons.beanutils.BeanUtils;
import java.lang.reflect.InvocationTargetException;
import org.springframework.format.annotation.DateTimeFormat;
import com.fasterxml.jackson.annotation.JsonFormat;
import java.io.Serializable;
import java.util.Date;
import com.utils.DateUtil;

/**
* 宽带
* 后端返回视图实体辅助类
* （通常后端关联的表或者自定义的字段需要返回使用）
*/
@TableName("kuandai")
public class KuandaiView extends KuandaiEntity implements Serializable {
	private static final long serialVersionUID = 1L;

	//当前表
	/**
	* 宽带类型的值
	*/
	@ColumnInfo(comment="宽带类型的字典表值",type="varchar(200)")
	private String kuandaiValue;




	public KuandaiView() {

	}

	public KuandaiView(KuandaiEntity kuandaiEntity) {
		try {
			BeanUtils.copyProperties(this, kuandaiEntity);
		} catch (IllegalAccessException | InvocationTargetException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}



	//当前表的
	/**
	* 获取： 宽带类型的值
	*/
	public String getKuandaiValue() {
		return kuandaiValue;
	}
	/**
	* 设置： 宽带类型的值
	*/
	public void setKuandaiValue(String kuandaiValue) {
		this.kuandaiValue = kuandaiValue;
	}




	@Override
	public String toString() {
		return "KuandaiView{" +
			", kuandaiValue=" + kuandaiValue +
			"} " + super.toString();
	}
}
