    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt"
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '报修状态' : ''">
            <el-select v-model="searchForm.baoxiuZhuangtaiTypes" placeholder="请选择报修状态">
              <el-option label="=-请选择-=" value=""></el-option>
              <el-option
                  v-for="(item,index) in baoxiuZhuangtaiTypesSelectSearch"
                  v-bind:key="index"
                  :label="item.indexName"
                  :value="item.codeIndex">
                <!--lable是要展示的名称-->
        ...
        <el-row class="ad"
          ...
          <el-form-item>
            ...
            <a style="text-decoration:none" class="el-button el-button--success"
               ...
               href="http://localhost:8080/kuandaiyewuguanlixitong/upload/baoxiuMuBan.xls"
            >批量导入报修数据模板</a>
            &nbsp;
            <el-upload
                v-if="isAuth('baoxiu','导入导出')"
                ...
                :action="getActionUrl"
                :on-success="baoxiuUploadSuccess"
                :on-error="baoxiuUploadError"
            ...
            &nbsp;
            <!-- 导出excel -->
            <download-excel v-if="isAuth('baoxiu','导入导出')" style="display: inline-block"
                            class="export-excel-wrapper" :data="dataList" :fields="json_fields" name="baoxiu.xls">
              <!-- 导出excel -->
              <el-button
                  type="success"
                  icon="el-icon-download"
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                           prop="baoxiuZhuangtaiTypes"
                           header-align="center"
                           label="报修状态">
            <template slot-scope="scope">
              {{ scope.row.baoxiuZhuangtaiValue }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" :align="contents.tableAlign"
            ...
            <template slot-scope="scope">
              ...
              <el-button v-if="isAuth('baoxiu','查看')" type="success" icon="el-icon-tickets" size="mini"
                         @click="addOrUpdateHandler(scope.row.id,'info')">详情
              </el-button>
              <el-button v-if="isAuth('baoxiu','修改')" type="primary" icon="el-icon-edit" size="mini"
                         @click="addOrUpdateHandler(scope.row.id)">修改
              </el-button>
              <el-button v-if="isAuth('baoxiu','删除')" type="danger" icon="el-icon-delete" size="mini"
                         @click="deleteHandler(scope.row.id)">删除
              </el-button>
              <el-button v-if="isAuth('baoxiu','修改')&& scope.row.baoxiuZhuangtaiTypes==2" type="primary" icon="el-icon-edit" size="mini"
                         @click="fenpei(scope.row.id)">分配
              </el-button>
  ...
  methods: {
    chartDialog() {
      ...
      let params = {
        ...
        thisTable: {//当前表
          tableName: 'baoxiu',//当前表表名,
          sumColum: 'baoxiu_number', //求和字段
          date: 'insert_time',//分组日期字段
          // string : 'baoxiu_name',//分组字符串字段
          // types : 'baoxiu_types',//分组下拉框字段
        },
        // joinTable : {//级联表（可以不存在）
    ...
    getDataList() {
      ...
      params['baoxiuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
    ...
    // 导入功能上传文件成功后调用导入方法
    baoxiuUploadSuccess(data) {
      let _this = this;
      _this.$http({
        url: "baoxiu/batchInsert?fileName=" + data.file,
        method: "get"
      }).then(({data}) => {
        if (data && data.code === 0) {
          _this.$message({
