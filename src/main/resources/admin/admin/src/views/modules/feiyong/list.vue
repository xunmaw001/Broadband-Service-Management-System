        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '缴费' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.feiyongName" placeholder="缴费" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '缴费类型' : ''">
                        <el-select v-model="searchForm.feiyongTypes" placeholder="请选择缴费类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in feiyongTypesSelectSearch"
                               v-bind:key="index"
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '缴费状态' : ''">
                        <el-select v-model="searchForm.feiyongZhuangtaiTypes" placeholder="请选择缴费状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in feiyongZhuangtaiTypesSelectSearch"
                               v-bind:key="index"
                               :label="item.indexName"
                               :value="item.codeIndex">
                            <!--lable是要展示的名称-->
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/kuandaiyewuguanlixitong/upload/feiyongMuBan.xls"
                        >批量导入宽带缴费数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('feiyong','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="feiyongUploadSuccess"
                                :on-error="feiyongUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('feiyong','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入宽带缴费数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('feiyong','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "feiyong.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="缴费类型">
                        <template slot-scope="scope">
                            {{scope.row.feiyongValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="feiyongZhuangtaiTypes"
                                      header-align="center"
                                      label="缴费状态">
                        <template slot-scope="scope">
                            {{scope.row.feiyongZhuangtaiValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="年月">
                        <template slot-scope="scope">
                            {{scope.row.feiyongTime}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="feiyongOldMoney"
                                   header-align="center"
                                   label="缴费金额">
                        <template slot-scope="scope">
                            {{scope.row.feiyongOldMoney}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('feiyong','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('feiyong','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('feiyong','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //级联表下拉框搜索条件
    //当前表下拉框搜索条件
              feiyongTypesSelectSearch : [],
              feiyongZhuangtaiTypesSelectSearch : [],
            form:{
                id : null,
                yonghuId : null,
                feiyongName : null,
                feiyongTypes : null,
                feiyongZhuangtaiTypes : null,
                feiyongTime : null,
                feiyongOldMoney : null,
                feiyongDelete : null,
                insertTime : null,
                createTime : null,
            },
            ...
            json_fields: {
                ...
                //本表字段
                     '缴费': "feiyongName",
                     '缴费类型': "feiyongValue",
                     '缴费状态': "feiyongZhuangtaiValue",
                     '年月': "feiyongTime",
                     '缴费金额': "feiyongOldMoney",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'feiyong',//当前表表名,
                        sumColum : 'feiyong_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'feiyong_name',//分组字符串字段
                        // types : 'feiyong_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.feiyongName!= '' && this.searchForm.feiyongName!= undefined) {
                    params['feiyongName'] = '%' + this.searchForm.feiyongName + '%'
                }
         
                if (this.searchForm.feiyongTypes!= '' && this.searchForm.feiyongTypes!= undefined) {
                    params['feiyongTypes'] = this.searchForm.feiyongTypes
                }
         
                if (this.searchForm.feiyongZhuangtaiTypes!= '' && this.searchForm.feiyongZhuangtaiTypes!= undefined) {
                    params['feiyongZhuangtaiTypes'] = this.searchForm.feiyongZhuangtaiTypes
                }
                                        
                params['feiyongDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=feiyong_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.feiyongTypesSelectSearch = data.data.list;
                    }
                });
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=feiyong_zhuangtai_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.feiyongZhuangtaiTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            feiyongUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "feiyong/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
